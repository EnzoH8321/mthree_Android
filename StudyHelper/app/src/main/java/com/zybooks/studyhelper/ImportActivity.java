...
public class ImportActivity extends AppCompatActivity {
    ...
    private final StudyFetcher.OnStudyDataReceivedListener mFetchListener = new StudyFetcher.OnStudyDataReceivedListener() {
        @Override
        ...
        public void onQuestionsReceived(Subject subject, List<Question> questionList) {
            if (!questionList.isEmpty()) {
                ...
                Toast.makeText(getApplicationContext(), subject.getText() + " imported successfully", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(getApplicationContext(), subject.getText() + " contained no questions", Toast.LENGTH_SHORT).show();
            }
        }

        @Override
        public void onErrorResponse(VolleyError error) {
            Toast.makeText(getApplicationContext(), "Error loading subjects. Try again later.", Toast.LENGTH_LONG).show();
        ...
        for (int i = 0; i < numCheckBoxes; i ++) {
            ...
            if (checkBox.isChecked()) {
                ...
                if (studyDb.subjectDao().getSubjectByText(subject.getText()) == null) {
                    ...
                    Toast.makeText(this, subject.getText() + " is already imported.", Toast.LENGTH_SHORT).show();
                }
            }
